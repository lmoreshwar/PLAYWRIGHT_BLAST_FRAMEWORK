...
test.describe('Unauthenticated Inventory Access', () => {
    test('TC_031 Unauthenticated direct access to inventory is guarded @ProductSortingOnTheInventoryPage @Negative', async ({
        loginModule,
        loginPage,
        inventoryPage,
    }) => {
        ...
        await expect(inventoryPage.productsTitle()).not.toBeVisible();
        await expect(inventoryPage.productItemByName(testData.products.backpack)).not.toBeVisible();
        await expect(inventoryPage.shoppingCartLink()).not.toBeVisible();
